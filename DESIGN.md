...
- *Rule controller's bottom-right panel, arbitrary per-rule params*: not yet addressed. Each rule currently exposes only the four fixed weights (mutation/embellish/fill/hold); a generic per-rule parameter manifest (extending §2's declarative `MutationParam` pattern to rules, since different rules will want different 0-2 extra knobs) is designed but not built.
