...
- *No way to run independent, non-branching "sections" (A/B/etc.)*: not yet addressed. Today there is exactly one persistent `LineageTree`/head per plugin instance; `BRANCH` creates a sibling off the current head's *parent*, which still shares ancestry rather than being a genuinely independent tree. This is an open architecture question — how session/tree scoping should change to support independent sections — not yet resolved.
