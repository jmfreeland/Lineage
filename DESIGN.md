...
- *Humanization has no timing component*: confirmed — `velocityHumanize` and the vocabulary-informed style both currently vary velocity (and, when a vocabulary is loaded, timing per-note via vocabulary lookup), but the always-on flat humanize path is velocity-only. Not yet addressed.
- *No visualization of humanization's impact in the current bars*: a `previewFlagHumanized` bit already exists per rendered note but isn't distinctly visualized. Not yet addressed.
