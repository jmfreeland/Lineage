...
- *Persistent session state*: `src/runtime.ts` is a JS module loaded once and resident in the plugin's QuickJS context for its whole lifetime — module-level state there IS the plugin's session memory across `processBlock()` calls now, not recreated per block. Played notes are captured bar-by-bar (using the same real transport info as above) into a genuine `LineageTree` that grows as you play — the plugin now has memory of a session. A new `RecordedProvenance` variant (§3's provenance union) marks these nodes as "this is what got played," distinct from a mutation- or live-session-derived node. This captured history doesn't drive playback yet (looping/evolving a captured bar back out) — that's real MIDI scheduling, a distinct and larger piece of work, left for later. `JsEngine::getSessionInfo()` and `__lineageGetSessionInfo()` exist to make this persistence observable/testable without a DAW; `plugin/Tests/BridgeTest.cpp` verifies node count grows across a simulated bar boundary on the same engine instance.
- *Broader coverage*: `ghostNote` is now chained after `velocityHumanize` in the bridge pipeline, gated by a new `juce::AudioParameterBool` ("Ghost Notes", default off since it changes note count) plus its own `macroEligible` "probability" param as a `juce::AudioParameterFloat`. Wiring in a note-count-changing mutation surfaced (and fixed) a real bug: `JsEngine::processBlock()`'s read-back loop was bounded by the *input* event count, silently dropping any notes past that if a mutation's output array was longer — it now reads the actual returned array length. It also surfaced a genuine scheduling boundary: a ghost note's computed sample position can fall outside the current block (its offset may land before the block started or after it ends); `PluginProcessor` now converts each output note's beat position back to a sample offset (via new `blockStartBeat`/`sampleRate` fields on `Transport`) and drops anything outside `[0, numSamples)` rather than misfiring it at a clamped, wrong position. Cross-block scheduling — actually placing such a note in the block where it belongs — isn't built; this MVP is honest about dropping it instead of getting it subtly wrong.

What's still not covered, and still a real next step rather than a formality: playback driven from the captured session history (the lineage tree above is memory, not yet a loop), cross-block MIDI scheduling (ghost notes landing outside their block are dropped, not deferred), and most of what the engine can do — fills, embellishments, other mutation types, the live-loop session itself — still isn't reachable from inside a DAW.

**Visual seed-groove editor and local build tooling (decided, MVP built):**

- *Step sequencer*: `plugin/Source/StepSequencerComponent` — a fixed 3-lane (kick/snare/hihat) × 16-step grid in the plugin editor, not a general lane/length authoring UI, since the goal was a way to program *a* starting groove, not a full groove editor. Toggling a step calls `LineageAudioProcessor::setSeedGroove()` immediately (no separate "apply"), which calls a new `__lineageSetSeedGroove` bridge function: `src/runtime.ts` builds a real `Groove` from the authored steps and **resets** the session to a fresh `LineageTree` rooted at it — a hard reset ("program a starting groove"), not a branch off whatever was captured before. This is deliberately scoped short of autonomous playback: the seeded groove becomes the session's root and is ready for when playback/live-loop mode is built, but the plugin doesn't loop or play it out on its own yet — you still have to play into it (or wait for the deferred scheduling work above) to hear anything change.
...
- *Local build tooling*: `plugin/build.sh` — `npm install` (for the esbuild runtime bundle), configure, build the VST3 target, and (by default) install straight into the local VST3 folder (`~/Library/Audio/Plug-Ins/VST3` on macOS, `~/.vst3` on Linux) so it shows up in Bitwig/Ableton without manual copying. `--no-install` to skip that, `--clean` to wipe the build dir. Verified in this session on Linux (both install and `--no-install` paths); macOS/Windows builds themselves can't be verified from this sandbox — see the note below.

**Known gap:** everything in this document has only ever been verified headless, in a Linux sandbox with no display, audio hardware, or DAW — `Lineage_VST3` compiles and `LineageBridgeTest` passes, but the plugin has never actually been loaded into Bitwig or Ableton. That's the real next validation step, on real hardware, not something further sandbox work can substitute for.
