...
- *No way to run independent, non-branching "sections" (A/B/etc.)* (decided, MVP built): addressed. `src/runtime.ts` now holds a `Map<string, Section>` (each with its own `LineageTree`, head, live-capture state, and rule-generation counter) instead of one global tree, plus an `activeSectionId` pointer — genuinely independent history per section, not a `BRANCH` (which still shares ancestry with the current head's parent). Exactly one section is active/audible at a time; `setSeedGroove()`, `evolveWithRule()`, and all playback (`renderPlaybackBlock`/`renderPlaybackPreview`/`processBlock`) operate on whichever section is active, via `__lineageCreateSection`/`__lineageListSections`/`__lineageSelectSection`/`__lineageDeleteSection`. `JsEngine`/`PluginProcessor` expose matching C++ methods, and a new `SectionBarComponent` in the plugin header renders sections as radio-style tabs (auto-named A, B, C, … in creation order) with a `+` to add and an `x` to delete (guarded against deleting the last one). `BridgeTest.cpp` proves genuine independence directly: seeding and evolving a second section leaves the first section's tree, head, and node count completely unchanged. Two deliberate simplifications, both flagged in code: (1) creating, selecting, or deleting a section resets automatic evolution to paused, the same way loading a seed already did — each section's own auto-evolution schedule isn't independently remembered across a switch yet; (2) switching to an *already-populated* section does not resync the seed editor grid or evolution-tree canvas to reflect that section's real content (both are safe no-ops chosen specifically to avoid a worse failure mode — auto-reloading the grid on switch would fire the existing "every edit hard-resets the tree" behavior and could silently destroy that section's real history the moment you switched to look at it). The underlying audio/lineage state is fully correct and isolated per section regardless; this is a display-only gap, not a data-integrity one. A UI round-trip that reads an arbitrary section's groove/tree back into those two panels is a clear next step.
...
- *Arranger tabs and macros don't do anything*: confirmed, already documented above as scaffolding — no new gap.
