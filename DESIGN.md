...
What this MVP deliberately does *not* yet cover: host transport/tempo sync (there's no bar/tempo mapping — the bridge treats each block's notes as a flat throwaway single-bar lane), persisted lineage/live-loop session state inside the plugin, or host-exposed parameters (§9's `macroEligible` params aren't wired to real VST3 parameters yet). Each is a real next step, not a formality.
