...
What's still not covered, and still a real next step rather than a formality: cross-block scheduling for mutation-created ghost notes (generated groove note-offs are scheduled, but ghost notes landing outside their input block are still dropped), and most of what the engine can do — fills, embellishments, other mutation types, and the live-loop session itself — still isn't reachable from inside a DAW.

**Visual seed-groove editor and local build tooling (decided, MVP built):**

- *Step sequencer*: `plugin/Source/StepSequencerComponent` — a fixed 3-lane (kick/snare/hihat) × 16-step grid in the plugin editor, not a general lane/length authoring UI, since the goal was a way to program *a* starting groove, not a full groove editor. Toggling a step calls `LineageAudioProcessor::setSeedGroove()` immediately (no separate "apply"), which calls `__lineageSetSeedGroove`: `src/runtime.ts` builds a real `Groove` from the authored steps and **resets** the session to a fresh `LineageTree` rooted at it — a hard reset ("program a starting groove"), not a branch off whatever was captured before. That current head now loops autonomously against the DAW transport and is emitted as MIDI with cross-block note-off scheduling.
